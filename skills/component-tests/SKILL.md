...
1. **`repo-good-<slice>`:** минимальный репозиторий, в котором новый check
...
2. **`repo-bad-<slice>`:** добавь к minimal-базе ровно один элемент,
   триггерящий нужную ветку отказа. Один элемент — одна ветка.
3. Для каждой дополнительной ветки отказа — отдельный вариант `repo-bad`
...
1. Выпиши пункты контракта — подкоманды/эндпоинты, поля отчёта по схеме,
...
8. **Фикстуры изолированы на слайс** — `repo-good-<slice>` и `repo-bad-<slice>`.
   Фикстуры других слайсов не используются. Фикстура `assess` — отдельная.
9. **Число сценариев = формула** — лишние удалены; каждый сценарий триггерит
