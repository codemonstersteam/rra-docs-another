...
// ParseArgs — ингресс-адаптер слайса readability: только парсинг флагов и
// позиционного аргумента. args — os.Args[2:] (после имени подкоманды "readability").
...
func ParseArgs(args []string, stderr io.Writer) (domain.Request, error) {
	fs := flag.NewFlagSet("readability", flag.ContinueOnError)
	fs.SetOutput(stderr)
