...
**Закрыто:** #1, #2 (PR #15); #5 — T1 (PR #19) + T1b (PR #20); #3, #4 — T2/T3 (PR #21).
doc_drift на `passkey-demo-api`: **194 → 152 → 68**. **Остаётся:** **T1c** — сузить
проверку расширения до allowlist (остаток #5: OpenAPI `paths.*`, Go-символы
`pkg.Symbol`, `~/`-абсолюты всё ещё ложно матчатся через «любой `.alnum`») —
последний штрих точности L6a.
...
#### E16-T1c — fix: расширение по allowlist, не «любой `.alnum`» (остаток #5)
