...
**Смягчение гейта L5 — ✅ сделано (S7-soften, ADR 0004).** ИИ-проверка
пригодности (L5) запускается при наличии документации (`hasDocs`), а не только
когда вся статика L4 прошла; `FAIL` на L4 не пропускает L5, а ограничивает итог
сверху до `PARTIAL` (`capL5ByL4` внутри `mergeOutcomes`; `shortCircuit` удалён).
Цель — прогон на реальных проектах и сбор статистики L4/L5 для калибровки.
