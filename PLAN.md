...
- `api-specification/cli.md` — семь подкоманд, флаги (вкл. `--llm-provider`,
  `--llm-base-url`, `--llm-model`), коды возврата `0/1/2`.
...
| `LLMClient` | LLM: Anthropic native / OpenAI-совм. (ADR 0001) | `Simulate(JTBDPrompt) -> (JTBDResult, error)` |
